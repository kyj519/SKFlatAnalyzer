...
void Vcb_Mu::initializeAnalyzer()
{
  ...
  int syst_checker = 0;
  if (run_jes_down == true)
    syst_checker++;
  if (run_jes_up == true)
    syst_checker++;
  if (run_jec_down == true)
    syst_checker++;
  if (run_jec_up == true)
    syst_checker++;
  ...
  for (auto &trigger_name : vec_mu_trig)
    cout << "[Vcb_Mu::initializeAnalyzer] Iso Muon Trigger Name = " << trigger_name << endl;
...
void Vcb_Mu::executeEvent()
{
  ...
  if (run_jes_down)
    ...
    param.syst_ = AnalyzerParameter::JetResUp;
  else if (run_jec_down)
...
void Vcb_Mu::executeEventFromParameter(AnalyzerParameter param)
{
  ...
  if (!IsData && run_reco_eval)
  {
    ...
    if (!fitter_driver->Check_Status())
    {
      ...
      if (chk_included)
        FillHist(param.Name + "/KF_Eval_Fail", n_sel_jet, n_b_jet, 1, 10, 0, 10, 8, 0, 8);
    ...
    if (fitter_correct)
      reco_eval_tree_correct->Fill();
    else
      reco_eval_tree_wrong->Fill();
