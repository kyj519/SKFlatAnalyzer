#include "PUJets_jer.h"

PUJets_jer::PUJets_jer(){
...
void PUJets_jer::initializeAnalyzer(){
  ...
  // else if(DataYear==2017){
  //   jetPtResolutionPath   = BASE_PATH + "2017/jme/";
  //   jetPtResolutionSFPath = BASE_PATH + "2017/jme/";
  // }
  // else if(DataYear==2016){
  //   jetPtResolutionPath   = BASE_PATH + "2016/jme/";
  //   jetPtResolutionSFPath = BASE_PATH + "2016/jme/";
  // }
  else{
...
void PUJets_jer::executeEventFromParameter(AnalyzerParameter param){
	Event ev = GetEvent();
  ...
  double jmePtJER = jet_resolution.getResolution({{JME::Binning::JetPt, jets.at(0).Pt()}, {JME::Binning::JetEta, jets.at(0).Eta()}, {JME::Binning::Rho, Rho}});
  double jmePtJERSF = jet_resolution_sf.getScaleFactor({{JME::Binning::JetPt, jets.at(0).Pt()},{JME::Binning::JetEta, jets.at(0).Eta()}}, Variation::NOMINAL);
  double dPhi_1 = ZCand_reverse.DeltaPhi(jets.at(0));
  bool toggle = true;
  if(jets.at(0).Pt() < 10 || jets.at(0).Pt() >= 50) return;


  double jer_sigma = 1.5;
  //if((ZPt/JetPt)>1.5) return;
  if( fabs(ZPt/JetPt-1) > jer_sigma * jmePtJER * jmePtJERSF ) toggle = false;
...
unsigned int PUJets_jer::getBinID(const double eta, const double Pt){
  unsigned PtNum = 0;
  unsigned etaNum = 0;
...
std::vector<TString> PUJets_jer::getHistKey(bool isDATA, bool pt_balanced, unsigned int binID, int PUID){
