  ...
  for (unsigned int i = 0; i < muons.size(); i++)
  {
    ...
    else
    {
      ...
      if (!IsDATA)
      {
        ...
        if (DataYear == 2016)
          ptvalues = muonGE->GeneralizedEndpointPt(this_tunep4.Pt(), this_tunep4.Charge(), this_tunep4.Eta(), this_tunep4.Phi() * 180. / M_PI, event);
        else
          ptvalues = muonGEScaleSyst->GEPt(DataYear, this_tunep4.Pt(), this_tunep4.Eta(), this_tunep4.Phi(), this_tunep4.Charge());
...
bool AnalyzerCore::PassMETFilter()
{
  ...
  if (!IsFastSim)
  {
    if (!Flag_globalSuperTightHalo2016Filter)
      return false;
  }
  if (!Flag_HBHENoiseFilter)
    return false;
  if (!Flag_HBHENoiseIsoFilter)
    return false;
  ...
  if (!Flag_BadPFMuonDzFilter)
    return false;
  if (!Flag_eeBadScFilter)
    return false;
  if (DataYear >= 2017)
  {
    if (!Flag_ecalBadCalibFilter)
      return false;
  }
...
double AnalyzerCore::GetPrefireWeight(int sys)
{
  ...
  {
    if (sys == 0)
      return L1PrefireReweight_Central;
    else if (sys > 0)
      return L1PrefireReweight_Up;
    else
      return L1PrefireReweight_Down;
  }
...
double AnalyzerCore::GetPDFWeight(LHAPDF::PDF *pdf_, bool chk_print)
{
  ...
  if (chk_print)
  {
    cout << "genWeight_id1 = " << genWeight_id1 << ", genWeight_X1 = " << genWeight_X1 << ", genWeight_id2 = " << genWeight_id2 << ", genWeight_X2 = " << genWeight_X2 << ", genWeight_Q  = " << genWeight_Q << ", pdf_1 = " << pdf_1 << ", pdf_2 = " << pdf_2 << endl;
  }
...
{
  double weight = GetPDFWeight(pdfReweight->NewPDF) / GetPDFWeight(pdfReweight->ProdPDF);
  if (weight < -1e3)
  {
    cout << "AnalyzerCore::GetPDFReweight " << weight << " PDF_weight_new = " << GetPDFWeight(pdfReweight->NewPDF) << ", PDF_weight_prob = " << GetPDFWeight(pdfReweight->ProdPDF) << endl;
    cout << "PDF_New" << endl;
    GetPDFWeight(pdfReweight->NewPDF, true);
    cout << "PDF_Prob" << endl;
    GetPDFWeight(pdfReweight->ProdPDF, true);
    cout << endl;
    cout << endl;
  }
...
{
  if (syst == "As_Up")
    return GetPDFWeight(pdfReweight->NewPDFAlphaSUp) / GetPDFWeight(pdfReweight->ProdPDF);
  else if (syst == "As_Down")
    return GetPDFWeight(pdfReweight->NewPDFAlphaSDown) / GetPDFWeight(pdfReweight->ProdPDF);
  else
  {
...
double AnalyzerCore::projectedMET(TLorentzVector a, TLorentzVector b, Particle METv)
{
  ...
  if (fabs(a.DeltaPhi(METv)) < fabs(b.DeltaPhi(METv)))
  {
    ...
    return (METv.Pt() * TMath::Sin(fabs(b.DeltaPhi(METv))));
}
...
std::vector<Muon> AnalyzerCore::MuonWithoutGap(const std::vector<Muon> &muons)
{
  ...
  for (unsigned int i = 0; i < muons.size(); i++)
  {
    ...
    if (1.444 <= this_eta && this_eta < 1.566)
      continue;
...
std::vector<Muon> AnalyzerCore::MuonPromptOnly(const std::vector<Muon> &muons, const std::vector<Gen> &gens)
{
  ...
  for (unsigned int i = 0; i < muons.size(); i++)
  {
    if (GetLeptonType(muons.at(i), gens) <= 0)
      continue;
    out.push_back(muons.at(i));
  }
...
std::vector<Electron> AnalyzerCore::ElectronPromptOnly(const std::vector<Electron> &electrons, const std::vector<Gen> &gens)
{
  ...
  for (unsigned int i = 0; i < electrons.size(); i++)
  {
    if (GetLeptonType(electrons.at(i), gens) <= 0)
      continue;
    out.push_back(electrons.at(i));
  }
...
int AnalyzerCore::GetPrElType_InSameSCRange_Public(int TruthIdx, const std::vector<Gen> &TruthColl)
{
  ...
  if (TruthIdx < 2)
    return false;
  if (abs(TruthColl.at(TruthIdx).PID()) != 11)
    return false;
  if (TruthColl.at(TruthIdx).Status() != 1)
    return false;
  ...
  {
    if (TruthColl.at(it_gen).Status() != 1)
      continue;
    if (abs(TruthColl.at(it_gen).PID()) != 11)
      continue;
    if (fabs(TruthColl.at(TruthIdx).Eta() - TruthColl.at(it_gen).Eta()) > dEtaMax)
      continue;
    if (TruthColl.at(TruthIdx).DeltaPhi(TruthColl.at(it_gen)) > dPhiMax)
      continue;
...
int AnalyzerCore::GetLeptonType_Public(int TruthIdx, const std::vector<Gen> &TruthColl)
{
  ...
  if (TruthColl.at(TruthIdx).Status() != 1)
    return 0;
  if (!(fabs(TruthColl.at(TruthIdx).PID()) == 11 || fabs(TruthColl.at(TruthIdx).PID()) == 13))
    return 0;
  ...
  if (GrMotherIdx != -1)
  {
    grmom_history = TrackGenSelfHistory(TruthColl.at(GrMotherIdx), TruthColl);
...
TH1D *AnalyzerCore::JSGetHist1D(TString suffix, TString histname)
{
  ...
  else
  {
    ...
    if (mapitit != this_maphist.end())
      return mapitit->second;
  }
